...
class Trie{
    ...
    void insert_data(string name,string phone_number,Data add){
        ...
        for(int i=0;i<name.length();i++){
            if(cur->links[get_index(name[i])]==NULL){
                cur->links[get_index(name[i])]=new Node();
            }
            cur=cur->links[get_index(name[i])];
        }
        cur->v.push_back(add);
    ...
    void add_required_details(Node*cur,vector<Data>&answer){
        ...
        for(int i=0;i<37;i++){
            if(cur->links[i]!=NULL){
                add_required_details(cur->links[i],answer);
            }
        }
    }
